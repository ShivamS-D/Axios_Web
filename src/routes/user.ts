...
router.post('/signin',async (req:any,res:any)=>{
  ...
  const verification=await nacl.sign.detached.verify(data,
   new Uint8Array(signature.signature.data),
    ...
    // const hardCodedAddress='5Efn5gtWukbXRxGQGDs3TReEzqNbXt9dZufuJpFV2J9X'
    let user;
    try{
    ...
    const token=jwt.sign({
      userId:user?.id},"ashutosh",{expiresIn:60*60})
...
//   const {url,fields}=await createPresignedPost(s3Client,{Bucket:S3_BUCKET,Key:`fivver/${userId}/${Math.random()}/image.jpg`,
//   Conditions:[
